...
export function CourierCheckSubscriptionManagement() {
  ...
  const fetchOrders = async () => {
    ...
    if (error) {
      console.error('Error fetching courier check orders:', error);
    } else {
      setOrders((data || []) as CourierCheckOrder[]);
    }
    setLoading(false);
  };
  ...
  const approveOrder = async (order: CourierCheckOrder) => {
    ...
    try {
      ...
      toast({
        title: "✅ Approved",
        description: "Courier Check subscription activated for 1 year",
      });

      fetchOrders();
    } catch (error) {
      console.error('Error approving courier check order:', error);
      toast({
        title: "Error",
